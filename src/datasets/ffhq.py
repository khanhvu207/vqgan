...
class FFHQ(Dataset):
    def __init__(self, root, train=True, transform=None):
        self.root = root
        self.image_dir = os.path.join(root, "ffhq-dataset/thumbnails128x128")
        ...
        if train is True:
            self.indices = self.indices[: self.num_train]
        else:
            self.indices = self.indices[self.num_train :]
...
class FFHQDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_data = FFHQ(self.data_dir, train=True, transform=train_transforms)
        self.val_data = FFHQ(self.data_dir, train=False, transform=val_transforms)
